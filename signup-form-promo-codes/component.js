...
(RaiselyComponents, React) => {
	...
	class PaymentForm extends React.Component {
		...
		setPromoCode = (id, value) => {
			const values = this.props.getValues();
			console.log(values);
			const { codes } = values;
			const code = codes.find(c => c.code.toUpperCase() === value.toUpperCase());
			console.log(code);
			if (code) {
		...
		render() {
			...
			const registrationAmount = values.settings.validPromoCode ? Number(values.settings.promoCodeObject.fee) * 100 : config.registrationAmount;
			...
			return (
				<div className="signup-form__payment">
					...
					<Input type="text" label="Promo Code" value={values.settings.promoCode} id="promoCode" change={this.setPromoCode} />
					{values.settings.promoCode && (
						<div className={`promo-code promo-code--${values.settings.validPromoCode ? 'valid' : 'invalid'}`}>
							{values.settings.validPromoCode ? `The promo code "${values.settings.promoCode}" has been applied.` : `The promo code "${values.settings.promoCode}" isn't valid. Please check it.`}
						</div>
					)}
					<DonationForm
						{...this.props}
						...
						paymentButton={config.feeType === 'Registration' ? fee => `Pay Registration Fee (${fee})` : null}
					/>
					<div className="signup-form__nav">
